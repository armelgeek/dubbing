...
export interface TranslationProvider {
  id: string
  translate: (
    input: { segments: SegmentTiming[]; sourceLang: string; targetLangs: string[] },
    onProgress?: (lang: string, p: number) => void
...
class LocalTranslationProvider implements TranslationProvider {
  ...
  async translate(
    ...
    const outputs = await translationClient.translateSegments(
      { segments: input.segments, sourceLang: input.sourceLang, targetLangs: input.targetLangs },
      onProgress
    )
